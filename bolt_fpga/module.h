...
class Conv2dCoeff : public Module<Data> {
 public:
  ...
  Conv2dCoeff(Tensor<Data>& weight, int Co, int Ci, int H, int W, int kh,
              int stride, int padding, CryptoInfo crypto_info,
              BfvParameter param)
      : Module<Data>(),
        weight(weight),
        Co(Co),
    ...
    channel_per_poly = 0;
    int paddedH = H + 2 * padding, paddedW = W + 2 * padding;
    tile = 1;
    int Oc = 0, kh1 = kh / 2, Sw = W + 2 * padding, Sh = H + 2 * padding;

    for (int c = 0; c < Ci; c++) {
      Oc = c * (Sh + 2 * kh1) * (Sw + 2 * kh1) + (kh - 1) * (Sw + 2 * kh1) +
           kh - 1 + (Sw + 2 * kh1) * (Sh - 1) + Sw - 1;
      if (Oc < ct_info.N) {
        channel_per_poly++;
      } else {
        break;
    ...
    if (channel_per_poly == 0) {
      ...
      while (true) {
        ...
        Oc = (kh - 1) * (Sw + 2 * kh1) + kh - 1 + (Sw + 2 * kh1) * (Sh - 1) +
             Sw - 1;
        if (Oc < ct_info.N) {
          break;
        }
    ...
    num_poly = (Ci + channel_per_poly - 1) / channel_per_poly;
    ...
    if (party == sci::ALICE) {
      ...
      for (int co = 0; co < Co; co++) {
        for (int p = 0; p < num_poly; p++) {
          ...
          int O = (channel_per_poly - 1) * (Sh + 2 * kh1) * (Sw + 2 * kh1) +
                  (kh - 1) * (Sw + 2 * kh1) + kh - 1;

          for (int ci = 0; ci < channel_per_poly; ci++) {
            for (int ki = 0; ki < kh; ki++) {
              for (int kj = 0; kj < kh; kj++) {
                if (ci + channel_start < Ci && ci + channel_start >= 0) {
                  w_msg[O - ci * (Sh + 2 * kh1) * (Sw + 2 * kh1) -
                        ki * (Sw + 2 * kh1) - kj] =
                      weight.cached_data[co * Ci * kh * kh +
                                         (ci + channel_start) * kh * kh +
                                         ki * kh + kj];
                }
              }
            }
          ...
          weight_pt.push_back(
              ct_info.context_p->encode_coeffs_mul(w_msg, ct_info.level));
        }
      }
    }
  };
  ...
  vector<vector<Data>> pack_ac(Tensor<Data>& unpadded_ac) {
    Tensor<Data> activation(
        TensorShape(3, {unpadded_ac.tensorshape.shape[0],
    ...
    for (int c = 0; c < activation.tensorshape.shape[0]; ++c) {
      for (int h = 0; h < H; ++h) {
        for (int w = 0; w < W; ++w) {
          ...
          // account for padding
          int padded_index =
              c * paddedH * paddedW + (h + padding) * paddedW + (w + padding);
            ...
            for (int i = 0; i < Sh + 2 * kh1; i++) {
              for (int j = 0; j < Sw + 2 * kh1; j++) {
                if (c + channel_start < Ci &&
                    ...
                    c + channel_start >= 0 && i + ti * Sh - kh1 >= 0 &&
                    j + tj * Sw - kh1 >= 0) {
                  mask_msg[(Sh + 2 * kh1) * (Sw + 2 * kh1) * c +
                           (Sw + 2 * kh1) * i + j] =
                      activation
                          .cached_data[(W + 2 * padding) * (H + 2 * padding) *
                                           (c + channel_start) +
                                       (W + 2 * padding) * (i + ti * Sh - kh1) +
                                       (j + tj * Sw - kh1)];
                } else {
                  mask_msg[(Sh + 2 * kh1) * (Sw + 2 * kh1) * c +
                           (Sw + 2 * kh1) * i + j] = 0;
  ...
  Tensor<Data> depack_res(vector<vector<Data>>& res_poly) {
    int Sh = ((H + 2 * padding) + tile - 1) / tile;
    int Sw = ((W + 2 * padding) + tile - 1) / tile;
    int O = (channel_per_poly - 1) * (Sh + 2 * padding) * (Sw + 2 * padding) +
  ...
  Tensor<Data> forward(Tensor<Data>& input) {
    ...
    if (party == sci::ALICE) {
      ...
      for (int i = 0; i < ac_msg.size(); i++) {
        ac_pt.push_back(
            ct_info.context_p->encode_coeffs(ac_msg[i], ct_info.level));
      }
      for (int i = 0; i < ac_msg.size(); i++) {
      ...
      ac_msg = pack_ac(input);
      for (int i = 0; i < ac_msg.size(); i++) {
        ac_ct.push_back(ct_info.context_p->encrypt_asymmetric(
            ct_info.context_p->encode_coeffs(ac_msg[i], ct_info.level)));
      }
      for (int i = 0; i < ac_msg.size(); i++) {
        io->send_data(static_cast<void*>(ac_ct[i].serialize(param).data()),
                      ct_info.cipher_size);
      }
    }
#ifdef LOG_LAYERWISE
      ...
      for (int co = 0; co < Co; co++) {
        for (int ti = 0; ti < tile; ti++) {
          for (int tj = 0; tj < tile; tj++) {
            ...
            BfvPlaintext res_pt =
                ct_info.context_p->encode_coeffs(all_zeros, ct_info.level);
            BfvCiphertext res_ct =
                ct_info.context_p->encrypt_asymmetric(res_pt);
            for (int p = 0; p < num_poly; p++) {
              ct_info.context_p->add_inplace(
                  res_ct, ct_info.context_p->mult_plain_mul(
                              ac_ct[Arr3DIdx(tile, tile, num_poly, ti, tj, p)],
                              weight_pt[Arr2DIdx(Co, num_poly, co, p)]));
            }
        ...
        for (int ti = 0; ti < tile; ti++) {
          for (int tj = 0; tj < tile; tj++) {
            ...
            BfvPlaintext temp_buffer_pos =
                ct_info.context_p->encode_coeffs(pos_mask, ct_info.level);
            BfvPlaintext temp_buffer_neg =
                ct_info.context_p->encode_coeffs(neg_mask, ct_info.level);
            ct_info.context_p->add_plain_inplace(
                out_ct_ls[Arr3DIdx(Co, tile, tile, co, ti, tj)],
                temp_buffer_neg);
    ...
    if (party == sci::ALICE) {
      for (int co = 0; co < Co; co++) {
        for (int ti = 0; ti < tile; ti++) {
          for (int tj = 0; tj < tile; tj++) {
            vector<uint64_t> res = ct_info.context_p->decode_coeffs(
                out_mask[Arr3DIdx(Co, tile, tile, co, ti, tj)]);
      ...
      for (int co = 0; co < Co; co++) {
        for (int ti = 0; ti < tile; ti++) {
          for (int tj = 0; tj < tile; tj++) {
            vector<uint64_t> res =
                ct_info.context_p->decode_coeffs(ct_info.context_p->decrypt(
                    out_ct_ls[Arr3DIdx(Co, tile, tile, co, ti, tj)]));
            out_msg_ls.push_back(res);
          }
        }
...
class BoltMatmul : public Module<Data> {
 public:
  ...
  BoltMatmul(Tensor<Data>& weight, int d0_unp, int d1_unp, int d2_unp,
    ...
    if (party == sci::ALICE) {
      assert(weight.tensorshape.shape[0] == d1_unp &&
             weight.tensorshape.shape[1] == d2_unp);
      if (d1_unp % t != 0) {
      ...
      if (d2_unp % t != 0) {
        // round up d2 to the nearest multiple of 2t, this is due to the
        // rotation property of SIMD encoding 8192 slots need to be treated as
        // 2*4096 slots
    ...
    double p = std::log(std::sqrt(d2 * t / d1)) / std::log(2);
    input_rot = std::pow(2, static_cast<int>(p));
    if (d1 * input_rot + d2 * t / input_rot >
        2 * d1 * input_rot + d2 * t / (2 * input_rot)) {
        ...
        for (int j = 0; j < d2 / t; j++) {
          ...
          for (int k = 0; k < t; k++) {
            for (int l = 0; l < ct_info.N / 2; l++) {
              ...
              w_msg[l] =
                  weight.cached_data[(i * t + (l / d0 + input_rot - 1 -
                                               (k % input_rot)) %
                                                  t) *
                                         d2 +
                                     (j * t + (3 * t - 1 - l / d0 - input_rot +
                                               1 + (k % input_rot) - k) %
                                                  t)];  // i * d2 + j * t + k
              w_msg[l + ct_info.N / 2] =
                  weight.cached_data[(i * t + (l / d0 + input_rot - 1 -
            ...
            weight_pt.push_back(
                ct_info.context_p->encode_mul(w_msg, ct_info.level));
          }
        }
      }
    }
  ...
  vector<vector<Data>> pack_ac(Tensor<Data>& act) {
    assert(act.tensorshape.shape[0] == d0_unp &&
           act.tensorshape.shape[1] == d1_unp);
    ...
    for (int i = 0; i < d1 / t; i++) {
      ...
      for (int j = 0; j < ct_info.N / 2; j++) {
        ...
        ac_msg[j + ct_info.N / 2] =
            act.cached_data[(j % (d0 / 2) + d0 / 2) * d1 + i * t +
                            j / (d0 / 2)];
      }
  ...
  Tensor<Data> depack_res(vector<vector<Data>>& out_msg) {
    ...
    for (int i = 0; i < d2 / t; i++) {
      for (int j = 0; j < d0 * t; j++) {
        if (j < d0 * t / 2) {
          // TODO: the exact meaning of the following line
          out.cached_data[(j % (d0 / 2)) * d2 + i * t +
                          (t - j / (d0 / 2)) % t] = out_msg[i][j];
        } else {
          out.cached_data[(j % (d0 / 2) + d0 / 2) * d2 + i * t +
                          (2 * t - j / (d0 / 2)) % t] = out_msg[i][j];
        }
      }
    }
  ...
  Tensor<Data> forward(Tensor<Data>& input) {
    ...
    if (party == sci::ALICE) {
      ...
      for (int i = 0; i < ac_msg.size(); i++) {
        ac_ct.push_back(ct_info.context_p->encrypt_asymmetric(
            ct_info.context_p->encode(ac_msg[i], ct_info.level)));
      }
      for (int i = 0; i < ac_msg.size(); i++) {
        io->send_data(static_cast<void*>(ac_ct[i].serialize(param).data()),
                      ct_info.cipher_size);
    ...
    Conv2dpackingacInMilliSec += (acpacktimeend - acpacktimestart);
    std::cout << " Input packing takes: "
              << (acpacktimeend - acpacktimestart) / 1000.0 << " s"
              << std::endl;
#endif
#ifdef LOG_LAYERWISE
    ...
    if (party == sci::ALICE) {
      for (int i = 0; i < d2 / t; i++) {
        out_ct.push_back(
            std::move(ct_info.context_p->new_ciphertext(1, ct_info.level)));
      }
      // std::cout << "parameters: " << d0 << " " << d1 << " " << d2 << " " << t
      // < " " << input_rot << std::endl;
      std::cout << "0" << std::endl;
      FpgaDevice::init();
      std::cout << "1" << std::endl;
      ...
      vector<CxxVectorArgument> cxx_args = {
          {"w", &weight_pt}, {"ac", &ac_ct}, {"y", &out_ct}};
      fpga_project.run(ct_info.context_p, cxx_args);
    }
#ifdef LOG_LAYERWISE
    auto kernelcomputationend = TIMER_TILL_NOW;
    std::cout << "Time in sec for conv computation = "
              << ((kernelcomputationend - kernelcomputationstart) / 1000.0)
              << std::endl;

    Conv2dCompTimeInMilliSec += (kernelcomputationend - kernelcomputationstart);
#endif
    std::random_device rd;
    std::mt19937_64 gen(rd());
    std::uniform_int_distribution<uint64_t> distrib(0, ct_info.prime_mod - 1);
    ...
    if (party == sci::ALICE) {
      ...
      for (int i = 0; i < d2 / t; i++) {
        ...
        BfvPlaintext temp_buffer_pos =
            ct_info.context_p->encode(pos_mask, ct_info.level);
        BfvPlaintext temp_buffer_neg =
            ct_info.context_p->encode(neg_mask, ct_info.level);
        ct_info.context_p->add_plain_inplace(out_ct[i], temp_buffer_neg);
        out_share.push_back(move(temp_buffer_pos));
        io->send_data(static_cast<void*>(out_ct[i].serialize(param).data()),
                      ct_info.cipher_size);
      }
#ifdef LOG_LAYERWISE
      ...
      std::cout << "Time in sec for sending = "
                << ((send_end - send_start) / 1000.0) << std::endl;
#endif
    } else {
      for (int i = 0; i < d2 / t; i++) {
        vector<uint8_t> buffer(ct_info.cipher_size, 0);
    ...
    std::cout << "Time in sec for mask = " << ((maskend - maskstart) / 1000.0)
              << std::endl;
    ...
    if (party == sci::ALICE) {
      ...
      for (int i = 0; i < d2 / t; i++) {
        vector<uint64_t> res =
            ct_info.context_p->decode(ct_info.context_p->decrypt(out_ct[i]));
        out_msg.push_back(res);
      }
    }
    ...
    std::cout << "Time in sec for decrypt and unpack = "
              << ((decryptandunpackend - decryptandunpackstart) / 1000.0)
              << std::endl;
#endif
    return restensor;
  }
...
class ResNet18Coeff : public Module<Data> {
 public:
  CryptoInfo ct_info;
  Conv2dCoeff<Data> conv0, conv1, conv2, conv3, conv4, conv5, conv6, conv7,
      conv8, conv9, conv10, conv11, conv12, conv13, conv14, conv15, conv16,
      conv17, conv18, conv19;

  ResNet18Coeff(std::vector<std::string> weight_pth, CryptoInfo ct_info)
      : ct_info(ct_info) {
    // Initial block
    Tensor<Data> weight0(TensorShape(4, {64, 3, 7, 7}));
    weight0.load(weight_pth[0]);
    conv0 = Conv2dCoeff<Data>(weight0, 64, 3, 224, 224, 7, 2, 3, ct_info);
    // First Block
    Tensor<Data> weight1(TensorShape(4, {64, 64, 3, 3}));
    weight1.load(weight_pth[1]);
    conv1 = Conv2dCoeff<Data>(weight1, 64, 64, 56, 56, 3, 1, 1, ct_info);

    Tensor<Data> weight2(TensorShape(4, {64, 64, 3, 3}));
    weight2.load(weight_pth[2]);
    conv2 = Conv2dCoeff<Data>(weight2, 64, 64, 56, 56, 3, 1, 1, ct_info);
    Tensor<Data> weight3(TensorShape(4, {64, 64, 3, 3}));
    weight3.load(weight_pth[3]);
    conv3 = Conv2dCoeff<Data>(weight3, 64, 64, 56, 56, 3, 1, 1, ct_info);
    Tensor<Data> weight4(TensorShape(4, {64, 64, 3, 3}));
    weight4.load(weight_pth[4]);
    conv4 = Conv2dCoeff<Data>(weight4, 64, 64, 56, 56, 3, 1, 1, ct_info);

    // Second Block
    // Block 1
    Tensor<Data> weight5(TensorShape(4, {128, 64, 3, 3}));
    weight5.load(weight_pth[5]);
    conv5 = Conv2dCoeff<Data>(weight5, 128, 64, 56, 56, 3, 2, 1, ct_info);
    Tensor<Data> weight6(TensorShape(4, {128, 128, 3, 3}));
    weight6.load(weight_pth[6]);
    conv6 = Conv2dCoeff<Data>(weight6, 128, 128, 28, 28, 3, 1, 1, ct_info);
    Tensor<Data> weight7(TensorShape(4, {128, 64, 1, 1}));  // short cut
    weight7.load(weight_pth[7]);
    conv7 = Conv2dCoeff<Data>(weight7, 128, 64, 56, 56, 1, 2, 0, ct_info);

    // Block 2
    Tensor<Data> weight8(TensorShape(4, {128, 128, 3, 3}));
    weight8.load(weight_pth[8]);
    conv8 = Conv2dCoeff<Data>(weight8, 128, 128, 28, 28, 3, 1, 1, ct_info);
    Tensor<Data> weight9(TensorShape(4, {128, 128, 3, 3}));
    weight9.load(weight_pth[9]);
    conv9 = Conv2dCoeff<Data>(weight9, 128, 128, 28, 28, 3, 1, 1, ct_info);

    // Third Block
    // Block 1
    Tensor<Data> weight10(TensorShape(4, {256, 128, 3, 3}));
    weight10.load(weight_pth[10]);
    conv10 = Conv2dCoeff<Data>(weight10, 256, 128, 28, 28, 3, 2, 1, ct_info);
    Tensor<Data> weight11(TensorShape(4, {256, 256, 3, 3}));
    weight11.load(weight_pth[11]);
    conv11 = Conv2dCoeff<Data>(weight11, 256, 256, 14, 14, 3, 1, 1, ct_info);
    Tensor<Data> weight12(TensorShape(4, {256, 128, 3, 3}));  // short cut
    weight12.load(weight_pth[12]);
    conv12 = Conv2dCoeff<Data>(weight12, 256, 128, 28, 28, 1, 2, 0, ct_info);
    // Block 2
    Tensor<Data> weight13(TensorShape(4, {256, 256, 3, 3}));
    weight13.load(weight_pth[13]);
    conv13 = Conv2dCoeff<Data>(weight13, 256, 256, 14, 14, 3, 1, 1, ct_info);
    Tensor<Data> weight14(TensorShape(4, {256, 256, 3, 3}));
    weight14.load(weight_pth[14]);
    conv14 = Conv2dCoeff<Data>(weight14, 256, 256, 14, 14, 3, 1, 1, ct_info);

    // Fourth Block
    // Block 1
    Tensor<Data> weight15(TensorShape(4, {512, 256, 3, 3}));
    weight15.load(weight_pth[15]);
    conv15 = Conv2dCoeff<Data>(weight15, 512, 256, 14, 14, 3, 2, 1, ct_info);
    Tensor<Data> weight16(TensorShape(4, {512, 512, 3, 3}));
    weight16.load(weight_pth[16]);
    conv16 = Conv2dCoeff<Data>(weight16, 512, 512, 7, 7, 3, 1, 1, ct_info);
    Tensor<Data> weight17(TensorShape(4, {512, 256, 3, 3}));  // short cut
    weight17.load(weight_pth[17]);
    conv17 = Conv2dCoeff<Data>(weight17, 512, 256, 14, 14, 1, 2, 0, ct_info);

    // Block 2
    Tensor<Data> weight18(TensorShape(4, {512, 512, 3, 3}));
    weight18.load(weight_pth[18]);
    conv18 = Conv2dCoeff<Data>(weight18, 512, 512, 7, 7, 3, 1, 1, ct_info);
    Tensor<Data> weight19(TensorShape(4, {512, 512, 3, 3}));
    weight19.load(weight_pth[19]);
    conv19 = Conv2dCoeff<Data>(weight19, 512, 512, 7, 7, 3, 1, 1, ct_info);
  }
  Tensor<Data> forward(Tensor<Data> input, int sf) {
    ...
    // short cut
    Tensor<Data> ac15 = Relu<Data>(ac14 + conv7.forward(ac11), sf, 1);
    ...
    // short cut
    Tensor<Data> ac23 = Relu<Data>(ac22 + conv12.forward(ac19), sf, 1);
    ...
    // short cut
    Tensor<Data> ac31 = Relu<Data>(ac30 + conv17.forward(ac27), sf, 1);
