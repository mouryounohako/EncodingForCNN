 ...
 auto kernelcomputationstart= TIMER_TILL_NOW;
    ...
    if (party == sci::ALICE){
      ...
      for (int i = 0; i < weight_pt.size(); i++){
        //TODO: meaning of the following line
        int_ct.push_back(ct_info.context_p->mult_plain_mul(ac_ct[(input_rot - 1 - i % t % input_rot) * d1 / t + i / d2], weight_pt[i]));
      }
      for (int i = 1; i < d1 / t; i++){
        for (int j = 0; j < d2; j++){
          ct_info.context_p->add_inplace(int_ct[j], int_ct[j + i * d2]);
        }
      }
      for (int i = 0; i < d2 / t; i++){
        for (int j = 0; j < t; j++){
          if(j % input_rot != 0){
            ct_info.context_p->add_inplace(int_ct[i * t + j - j % input_rot], int_ct[i * t + j]);
          }
        }
