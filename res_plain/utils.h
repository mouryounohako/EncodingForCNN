...
class CryptoInfo {
 public:
  ...
  vector<std::shared_ptr<BfvContext>>
      context_par;  // Shared context used for parallel
                    // computation
  std::shared_ptr<BfvParameter> param;
  CryptoInfo(){}
  CryptoInfo(
      int party,std::shared_ptr<BfvParameter> &param, BfvContext *context_p = nullptr,
      int level = 1, int cipher_size = 0, int prime_mod = 1,
      int N = 8192,
      vector<std::shared_ptr<BfvContext>> context_par = {})
  {
    this->party = party;
  ...
  CryptoInfo(const CryptoInfo& other) {
    ...
    this->N = other.N;
    this->context_par = other.context_par;
...
Tensor<Data> MaxPool(int32_t C, int32_t H, int32_t W,
  ...
  Tensor<Data> out(TensorShape(3, {C, Ho, Wo}));
  MaxPool(1, Ho, Wo, C, kh, kh, padding, padding, padding,
          padding, stride, stride, 1, H, W, C,
          input.cached_data.data(), out.cached_data.data());
  return out;
}
#endif