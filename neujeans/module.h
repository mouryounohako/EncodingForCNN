...
class Conv2dNest : public Module<Data> {
 public:
  ...
  Conv2dNest(Tensor<Data>& weight, int Co, int Ci, int Hi, int Wi, int k, int s, int p, CryptoInfo crypto_info, BfvParameter param)
      : Module<Data>(),
        weight(weight),
        Co(Co),
        Ci(Ci),
  ...
  Tensor<Data> forward(Tensor<Data>& input){
    ...
    if (party == sci::ALICE){
      ...
      for (int i = 0; i < inrot - 1; i++){
        for (int j = 0; j < c_i; j++){
          ac_ct.push_back(ct_info.context_p->rotate(ac_ct[i * c_i + j], H * W));
        }
      }
      for (int i = 0; i < c_i; i++){
        for (int j = 0; j < c_o; j++){
          for (int l = 0; l < C; l++){
            int_ct.push_back(ct_info.context_p->mult_plain_mul(ac_ct[(inrot - 1 - l % inrot) * c_i + i], weight_pt[i * c_o * C + j * C + l]));
          }
        }
      }
      for (int i = 1; i < c_i; i++){
        for (int j = 0; j < c_o * C; j++){
          ct_info.context_p->add_inplace(int_ct[j], int_ct[j + i * c_o * C]);
        }
      }
      for (int i = 0; i < c_o; i++){
        for (int j = 0; j < C; j++){
          if(j % inrot != 0){
            ct_info.context_p->add_inplace(int_ct[i * C + j - j % inrot], int_ct[i * C + j]);
          }
        }
        out_ct.push_back(move(int_ct[i * C]));
        for (int j = inrot; j < C; j += inrot){
          out_ct[i] = ct_info.context_p->rotate(out_ct[i], H * W * inrot);
          ct_info.context_p->add_inplace(out_ct[i], int_ct[i * C + j]);
        }
      }
    }
#ifdef LOG_LAYERWISE
